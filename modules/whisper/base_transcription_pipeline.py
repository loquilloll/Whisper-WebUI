...
class BaseTranscriptionPipeline(ABC):
    def __init__(
        ...
        self.diarizer = Diarizer(model_dir=diarization_model_dir)
        self.vad = SileroVAD()
        self.music_separator = MusicSeparator(model_dir=uvr_model_dir, output_dir=os.path.join(output_dir, "UVR"))
    ...
    def run(
        ...
        *pipeline_params: tuple
        ...
        bgm_params, vad_params, whisper_params, diarization_params = (
            params.bgm_separation,
            params.vad,
            params.whisper,
            params.diarization,
        )
        ...
        if diarization_params.is_diarize:
            ...
            result, elapsed_time_diarization = self.diarizer.run(
                audio=origin_audio,
                use_auth_token=(
                    diarization_params.hf_token if diarization_params.hf_token else os.environ.get("HF_TOKEN")
                ),
    ...
    def is_sparse_api_supported():
        ...
        try:
            ...
            sparse_tensor = torch.sparse_coo_tensor(
                indices=torch.tensor([[0, 1], [2, 3]]), values=torch.tensor([1, 2]), size=(4, 4), device=device
            )
            return True
        except RuntimeError:
            return False
    ...
    @staticmethod
    def cache_parameters(params: TranscriptionPipelineParams, file_format: str = "SRT", add_timestamp: bool = True):
    ...
    def resample_audio(
        ...
        if isinstance(audio, str):
            ...
            # Ensure numpy array is writable for PyTorch
            audio = torch.from_numpy(np.array(audio, copy=True))
